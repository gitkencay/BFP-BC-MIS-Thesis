                                                ...
                                                <div class="panel-heading">
                                                    <ul class="panel-controls">
                                                        <button type="button" class="btn btn-info" data-toggle="modal"
                                                            data-target="#FSEC-Reg"><i class="fa fa-plus"></i>Add FSEC</button>
                                                    </ul>
                                                </div>
...
$query = $conn->query("SELECT * from `application` where application_type = 'FSEC' && year = '$year'") or die(mysqli_error());
while ($fetch = $query->fetch_array()) {
    $month = date("m", strtotime($fetch['month']));
    ?>
                                                                <tr>
                                                                    <td>
                                                                        <?php echo $fetch['year'] . '-' . $month . '-' . $fetch['application_no'] ?>
                                                                    </td>
                                                                    <td>
    ...
    ?>
                                                                    ...
                                                                    <td>
                                                                        <a href="DataEntry-FSEC.php?application_no=<?php echo $fetch['application_no'];?>" class="btn btn-sm btn-info">Edit</a>
                                                                    </td>
                                                                </tr>
                                                                <?php
}
                                        ...
                                        <div class="col-md-12">
                                            <div class="panel panel-default">
                                                <div class="panel-heading">
                                                    <ul class="panel-controls">
                                                        <button type="button" class="btn btn-info" data-toggle="modal"
                                                            data-target="#FSIC-Reg"><i class="fa fa-plus"></i>Add FSIC</button>
...
while ($fetch2 = $query2->fetch_array()) {
    ...
    ?>
                                                                    ...
                                                                    <td>
                                                                        <a href="DataEntry-FSIC.php?application_no=<?php echo $fetch2['application_no'];?>" class="btn btn-sm btn-info">Edit</a>
                                                                   </td>
                                                                </tr>
                                                                <?php
}
...
?>
        ...
        $("#fsec").validate({
            ...
            $(document).ready(function () {
                $("#pyear").on('change', function () {
                    var year = $(this).val();
                    window.location = 'DataEntry-AppReg.php?year=' + year;
                });
            });
        </script>
    <script>
    ...
    <script>
                 function myFunctionFSEC() {
                    document.getElementById("confirmationfsec").style.display = "block";
                    document.getElementById("fsecyes").style.display = "inline";
                    document.getElementById("fsecno").style.display = "inline";
                }
    </script>
