...
<body>
    ...
    ?>
            ...
            <div class="page-content-wrap">
                <div class="row">
                    <div class="col-md-3">
                        <div class="widget widget-default widget-carousel" onclick="location.href='Report-Transaction.php';">
...
?>
                    ...
                    <div class="col-md-3">
                        <div class="widget widget-default widget-item-icon" onclick="location.href='Report-Applicant-Summary.php';">
...
$fetch = $query->fetch_array();
?>
                        <div class="widget widget-default widget-item-icon" onclick="location.href='Report-Employee.php';">
                            <div class="widget-item-left">
                                <span class="fa fa-user"></span>
                            </div>
                            <div class="widget-data">
                ...
                <div class="row">
                <div class="col-md-4">
                        <div class="panel panel-default" onclick="location.href='Report-Transaction.php';">
                            <div class="panel-heading">
                                <div class="panel-title-box">
                                    <h4>Inspector Pending List</h4>
                                    <span>&nbspTop 10 Inspector with the highest pending list</span>
                                </div>
            ...
            $query = $conn->query("SELECT * FROM `employee` where `officer_type` = 'Inspector' && `year` = '$year' ORDER BY `employee`.`fullname` ASC limit 10") or die(mysqli_error());
            while($fetch = $query->fetch_array()){
                $querydate = $conn->query("SELECT * FROM `inspection_schedule` WHERE `inspection_date` != '$current' && `inspection_date` < '$current' && `status` = 'Pending' ORDER BY `inspection_schedule`.`inspectors` ASC limit 10") or die(mysqli_error());
                $fetchdate = $querydate->fetch_array();
                $status = $fetchdate['status'];

                $name = $fetch['fullname'];
                $query2 = $conn->query("SELECT *, COUNT(*) as numberofschedule FROM `inspection_schedule` WHERE `inspectors` = '$name' && `year` = '$year' && `status` = '$status' ORDER BY `inspection_schedule`.`inspectors` ASC limit 10") or die(mysqli_error());
...
$query10 = $conn->query("SELECT *, count(barangay_name) as total FROM `application` where `year` = '$year' GROUP BY barangay_name order by total DESC limit 10") or die(mysqli_error());
while ($fetch10 = $query10->fetch_array()) {
    ?>
                                    <tr>
                                        <td>
                                            <?php echo $fetch10['barangay_name'] ?>
                                        </td>
                                        <td>
                                            <?php echo $fetch10['total'] ?>
                                        </td>
                                    </tr>
                                    <?php
}
