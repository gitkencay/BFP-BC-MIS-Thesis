...
if(isset($_GET['year']))
{
    ...
    if(isset($_GET['year']))
    {
        $year=$_GET['year'];
                                        ...
                                        $query2 = $conn->query("SELECT * from `assessment` where month = '$month' && premium_tax != '0' && year = '$year' order by premium_tax DESC") or die(mysqli_error());
                                        while ($fetchb = $query2->fetch_array()) {
                                            $totaldue = number_format($fetchb['premium_tax'],2,".",",");
                                        ?>
