...
if(isset($_GET['year']))
{
    ...
    if(isset($_GET['year']))
    {
        $year=$_GET['year'];
                                        ...
                                        $query2 = $conn->query("SELECT * from `assessment` where month = '$month' && inspection_fee != '0' && year = '$year' order by inspection_fee DESC") or die(mysqli_error());
                                        while ($fetchb = $query2->fetch_array()) {
                                            $totaldue = number_format($fetchb['inspection_fee'],2,".",",");
                                        ?>
