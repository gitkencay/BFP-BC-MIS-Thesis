...
if(isset($_GET['year']))
{
    ...
    if(isset($_GET['year']))
    {
        $year=$_GET['year'];
                                        ...
                                        $query2 = $conn->query("SELECT * from `assessment` where month = '$month' && storage_clearance != '0' && year = '$year' order by storage_clearance DESC") or die(mysqli_error());
                                        while ($fetchb = $query2->fetch_array()) {
                                            $totaldue = number_format($fetchb['storage_clearance'],2,".",",");
                                        ?>
