...
if(isset($_GET['year']))
{
    ...
    if(isset($_GET['year']))
    {
        $year=$_GET['year'];
                                        ...
                                        $query2 = $conn->query("SELECT * from `assessment` where month = '$month' && construction_tax != '0' && year = '$year' order by construction_tax DESC") or die(mysqli_error());
                                        while ($fetchb = $query2->fetch_array()) {
                                            $totaldue = number_format($fetchb['construction_tax'],2,".",",");
                                        ?>
