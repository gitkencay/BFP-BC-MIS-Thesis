    ...
    <div class="page-container">
        ...
        <div class="page-content">
            ...
            <ul class="breadcrumb">
                ...
                <li class="active"><strong><mark>Inspector Pending Schedule</mark></strong></li>
            </ul>
...
if(isset($_GET['year']))
{
    $year=$_GET['year'];
                        ...
                        $query = $conn->query("SELECT * FROM `employee` where fullname = '$_GET[name]' && year = '$_GET[year]'") or die(mysqli_error());
                        $fetch = $query->fetch_array();
                        $year = $_GET['year'];
                        $name = $_GET['name'];
                        ?>
                        <h3> Inspector <?php echo $fetch['fullname']?> </h3>
                        <h3> &nbsp; List of Pending Schedule |  Year <?php echo $year?> </h3>
                    ...
                    <div class="panel-body">
                        <div id="print">
                            <div class="listheading" style="display: none">
                                ...
                                <br>
                                <h3> Inspector <?php echo $fetch['fullname']?> </h3>
                                <h3> List of Pending Schedule <?php echo $year?> </h3>
                                <br>
                                <br>
                            </div>
    ...
    $current = new dateTime();
                                        ...
                                        $query2 = $conn->query("SELECT * from `inspection_schedule` where `inspectors` = '$name' && `year` = '$year' && `status` = 'Pending'") or die(mysqli_error());
                                        while ($fetchb = $query2->fetch_array()) {
                                            $dateapplied = date(" D - F j Y", strtotime($fetchb['inspection_date']));
                                            $datetime1 = new DateTime($fetchb['inspection_date']);
